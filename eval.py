...
source_ds = Dummy_CIDA_Dataset(
    ...
    num_classes=16,
    num_unique_examples_per_class=1,
    domains=source_snrs,
    x_shape=(2,128)
)
...
all = Sequence_Aggregator([source_ds, target_ds])
all = wrap_in_dataloader(all)
...
# x_source = np.ones((2,128), dtype=np.float)
# for domain in target_snrs:
#     for y in range(16):
#         x = np.array(x_source * y * domain, dtype=np.single)
#         x = torch.from_numpy(x)
#         x = x.reshape((1,-1))

#         u = np.array(domain, dtype=np.single)
...
source_test_label_accuracy, source_test_label_loss, source_test_domain_loss = cida_tet_jig.test(all)
muh_device = torch.device(device)
...
for x,y,u,s in all:
    ...
    for i in range(x.shape[0]):
        y_c = y[i].cpu().item()
        u_c = u[i].cpu().item()
        pred_c = pred[i].cpu().item()
        
        if y_c != pred_c:
            print(y_c,u_c,pred_c)
